...
Optimisations implemented:
  ...
  2. Image decode cache     — decoded/resized numpy arrays are cached by
                              ...
                              identical payloads skip the base64 decode and
                              cv2 resize steps. YOLO inference still runs on
                              every request — this cache does NOT skip inference.
  3. Input resize to 480px  — all input images are resized to 480×480 before
                              inference regardless of original dimensions.
                              This acts as a server-side cap on oversized payloads
                              and reduces FLOPs vs 640px with minimal accuracy loss.
...
# ---------------------------------------------------------------------------
# Image decode cache
# Caches decoded + resized numpy arrays only.
# YOLO inference is NOT cached — it runs on every request.
# ---------------------------------------------------------------------------
_CACHE_MAX = 64
...
class WildlifeDetector:
    ...
    def _decode_and_cache(self, b64_string: str) -> np.ndarray:
        ...
        The decoded array is cached by MD5 hash so repeated identical payloads
        skip the decode + resize cost. YOLO inference is NOT cached here —
        _infer() is always called after this method.

        All images are resized to INFER_SIZE×INFER_SIZE regardless of their
        original dimensions, capping any oversized input before inference.
        """
        ...
        # Resize to inference resolution — caps any oversized input
    ...
    def decode_and_predict(self, b64_string: str) -> Dict[str, Any]:
        """
        Decode base64 image and run YOLO detection inference in one call.
        Returns a structured dict consumed by PredictResponse.
        YOLO inference runs on every request.
        """
    ...
    def decode_and_annotate(self, b64_string: str) -> str:
        """
        Decode base64 image, run YOLO detection, draw bounding boxes,
